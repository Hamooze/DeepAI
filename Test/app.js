...
function updateVisualization(state) {
  ...
  // Update clusters if enabled, otherwise remove them
  if (state.clusteringEnabled) {
      updateClustering(state);
  } else {
      removeClustering(); // Explicitly remove if not enabled
  }
...
function updateClustering(state) {
    const svg = d3.select("#graph");
    // Ensure we are selecting the correct group 'g' that contains the 'clusters' subgroup.
    // If 'g' is the main zoomable group created in createVisualization, this should be fine.
    const mainZoomableGroup = svg.select("g"); // This is the group that has the zoom transform
    let clusterGroup = mainZoomableGroup.select(".clusters");

    // If .clusters group doesn't exist (e.g., first time after DOM clear), create it.
    if (clusterGroup.empty()) {
        clusterGroup = mainZoomableGroup.append("g").attr("class", "clusters");
        console.log("Created .clusters group because it was empty.");
    }
    ...
    state.filteredData.nodes.forEach(node => {
        ...
        // Ensure nodes have x and y, which they should after simulation ticks
        if (node.x != null && node.y != null) {
            nodesByType[node.type].push(node);
        }
    });

    console.log("Nodes grouped by type for clustering:", JSON.parse(JSON.stringify(nodesByType)));

    Object.entries(nodesByType).forEach(([type, nodes]) => {
        if (nodes.length < 1) return; // Need at least 1 point to define a location, though hull needs 3 for area

        let points = nodes.map(node => [node.x, node.y]);
        console.log(`Points for type '${type}':`, JSON.parse(JSON.stringify(points)));

        // If less than 3 points, d3.polygonHull will return null or a line.
        // We can draw a small circle or rectangle around single/double nodes instead of a hull.
        if (points.length === 1) {
            clusterGroup.append("circle")
                .attr("class", "cluster-hull-point") // Different class for styling if needed
                .attr("cx", points[0][0])
                .attr("cy", points[0][1])
                .attr("r", 50) // Radius for the circle around a single node
                .style("fill", NODE_COLOR_MAP[type])
                .style("opacity", 0.25);
            return;
        } else if (points.length === 2) {
            // For two points, draw a thicker line or a bounding box
            const padding = 40;
            const minX = Math.min(points[0][0], points[1][0]) - padding;
            const maxX = Math.max(points[0][0], points[1][0]) + padding;
            const minY = Math.min(points[0][1], points[1][1]) - padding;
            const maxY = Math.max(points[0][1], points[1][1]) + padding;
            clusterGroup.append("rect")
                .attr("class", "cluster-hull-point")
                .attr("x", minX)
                .attr("y", minY)
                ...
                .style("fill", NODE_COLOR_MAP[type])
                .style("opacity", 0.25)
                .style("stroke", NODE_COLOR_MAP[type])
                .style("stroke-width", "2px");
            return;
        }

        // Try to compute the convex hull for 3+ points
        const hullPoints = d3.polygonHull(points);
        console.log(`Hull points for type '${type}':`, hullPoints);

        if (!hullPoints) {
            console.warn(`Could not compute hull for type '${type}', possibly collinear points.`);
            // Optionally draw bounding box for collinear points if desired
            if (points.length > 0) {
                ...
                clusterGroup.append("rect")
                    .attr("x", minX)
                    .attr("y", minY)
                    .attr("width", Math.max(10, maxX - minX)) // Ensure min width
                    .attr("height", Math.max(10, maxY - minY)) // Ensure min height
                    .style("fill", NODE_COLOR_MAP[type])
                    .style("opacity", 0.15)
                    .style("stroke", NODE_COLOR_MAP[type])
        ...
        const padding = 40; // Keep padding consistent
        const paddedHull = padHull(hullPoints, padding);

        clusterGroup.append("path")
            .attr("class", "cluster-hull")
            .attr("d", `M${paddedHull.join("L")}Z`)
            .style("fill", NODE_COLOR_MAP[type]) // Use .style() for d3 v7 for direct attributes
            .style("stroke", NODE_COLOR_MAP[type])
            // Opacity and stroke-width are handled by CSS class .cluster-hull
            ;
    });
}
