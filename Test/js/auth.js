...
// NEW: helper to read user data from either storage type
function getStoredUser() {
    return JSON.parse(localStorage.getItem('loggedInUser') || sessionStorage.getItem('loggedInUser'));
}
if (currentPageForAuthJs === 'login.html' || currentPageForAuthJs === 'signup.html') {
    ...
    const loggedInUserOnAuthPages = getStoredUser();
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', (event) => {
            ...
            if (loginSuccess) {
                ...
                try {
                    const payload = JSON.stringify(userDataForStorage);
                    if (rememberMeChecked) {
                        localStorage.setItem('loggedInUser', payload);
                        sessionStorage.removeItem('loggedInUser'); // ensure one source of truth
                    } else {
                        sessionStorage.setItem('loggedInUser', payload);
                        localStorage.removeItem('loggedInUser');
                    }
                } catch(e) {
                    console.error('Error persisting user data', e);
                }
